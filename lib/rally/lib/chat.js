...
module.exports = (bot, message, config, context, arg1, arg2) => {
    ...
    try {
        ...
        case 'list': {
		 ...
		 if ((data[i].L2RoadmapItem && data[i].L2RoadmapItem.Name) ||
			(data[i].L2RoadmapItem && data[i].L2RoadmapItem._refObjectName)) {
                    data[i].isL2Exists = 'Parent: Yes'
                }

                let attach = [{
                    fallback: 'List of User Stories and Defects',
                    color: clrFormat[data[i].ScheduleState],
                    title: data[i].FormattedID + '\t' + data[i].ScheduleState + '\t' + data[i].PlanEstimate + '\t' + data[i].isL2Exists,
